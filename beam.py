...
#IDÉE :
#très sembable a hill-climbing, mais on a plutot k etats actuels, et on prend comme successeurs les k meilleurs
#successeurs des k etats actuels
...
solutionCounter = 0
MAX = 50  #nombre d'états actuels (k)
f = open('beam.txt', 'a')  # fichier log de la performance
...
def swap(grid, coupleASwap):
    copy = grid.copy();
    temp = copy[coupleASwap[0]]
    copy[coupleASwap[0]] = copy[coupleASwap[1]]
    copy[coupleASwap[1]] = temp
    return copy
...
def solve(grid):
    ...
    grids = [] #on rempli aléatoirement les cases vide
    for i in range(0,MAX):
        grids = grids + [fill(originalGrid)]#on genere alléatoirement k etats actuels
    return beamSearch(grids, swaps) #algorithme de beam search

#But : prendre la liste des grilles actuelles et la liste des swaps possible, et tenté de résoudre le sudoku
#      selon le principe de l'algorithme de beam search
#      retourne la grille solvée, ou False si l'algorithme échoue
def beamSearch(grids, swaps):
    nbEtat = len(grids);
    nbSwaps = 0
    bestConflits = min(nombreConflits(g) for g in grids) #le nombre de conflits dans la grille initiallement
    while(True):
        ...
        successeurspossibles = []
        for s in swaps:
            for g in grids:
                successeurspossibles = successeurspossibles + [swap(g,s)]
            	nbSwaps += 1
        meilleurs = getMeilleurs(successeurspossibles, bestConflits)
        if len(meilleurs) == 0: #aucun successeur meilleur -> echec
            return(False,nbEtat, bestConflits, nbSwaps)  # on ne peu plus continuer, et on sait que l'on n'est pas à 0 conflit, donc le beam search échoue
        else: #on a des successeur meilleurs, donc on continue
            bestConflits = nombreConflits(meilleurs[0]) #meilleurs est sensé être trié
            grids = meilleurs
            nbEtat += len(meilleurs)

#but : à partir de la liste de l'ensemble des successeurs possible, prendre les k meilleurs
def getMeilleurs(list, conflit):
    list.sort(key=lambda x: nombreConflits(x))
    meilleurs = []
    for i in range(0, MAX):
        if(i == len(list) - 1):
            break
        if(nombreConflits(list[i]) >= conflit):
            break
        meilleurs = meilleurs + [list[i]]
    return meilleurs
