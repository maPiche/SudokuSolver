...
# HEURISTIQUE, recherche tabou : relaxation de contrainte lorsqu'on se trouve à un minimum local.
#                                chaque pair d'échange est ajouté à une liste tabou
...
f = open('validtaboo.txt', 'a')  # fichier log de la performance
...
def solve(grid):
    ...
    #####################################################################################################################display(grid)display(originalGrid)
    ...
    randomlyFilledGrid = fill(originalGrid,values)  # on rempli aléatoirement les cases vides
    return hillClimbing(randomlyFilledGrid, swaps,values)  # algorithme de hill climbing
...
def hillClimbing(grid, swaps, values):
    ...
    while (True):
        ...
        for s in shuffled(swaps):
            if s in tabouliste or not valid_swap(grid,s,values): continue  #exclus de la boucles les swap non-compatibles
        ...
        #####  Taboo Search ####
        if not onATrouveUnSwap:  # on n'a pas trouvé de swap réduisant les conflits, on considère les autres échanges
            for s in shuffled(swaps):
                if s in tabouliste or not valid_swap(grid, s, values): continue  # exclus de la boucles les swap non-compatibles
                filter_list = [s[0],s[1]]
                tabouliste = [tup for tup in tabouliste if not any(i in tup for i in filter_list)]
                tabouliste.append(s)          # on ajouté la paire à la liste tabou
                if len(tabouliste) > n : del tabouliste[0]  # maximum de n tuple dans la liste
                conflitsAvant = nombreConflits(swap(grid, s))
                nbTours += 1
                onATrouveUnSwap = True
                break
