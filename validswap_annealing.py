...
# HEURISTIQUE, sucesseur valide : les échange sont permis que entre des cases acceptant les valeurs échangées.
# Deux ensembles sont créés à partir de la case C courrante, et on choisi dans l'intersection de ces deux ensemble:
# une liste des cases acceptant la valeur dans C
# une liste des cases contenant une des valeur accepté par C
# Avant de faire un échange, on vérifier si a est dans B et b est dans A

# Recuit simulé ajouté à l'heuristique
...
f = open('validannealing.txt', 'a')  # fichier log de la performance
...
# une Heuristique de BestFit est employé ou l'on choisi de remplir d'abord les cases les plus contraignante en premier (le moins de valeurs possibles)
def fill(grid,values):
    filled = grid.copy()
    for bs in bigSquares:  # pour chaque carré 3x3
        bs2 = list(bs)
        possibilites = "123456789"
        #ordonne la liste d'un carré selon le nombre de valeurs possibles par case
        bs2 = [(s,values[s]) for s in bs2]
        bs2.sort(key=lambda x: len(x[1]))
        bs2 = [s[0] for s in bs2]
        for s in bs2:
        ...
        for s in bs2:
            if filled[s] == '.' or filled[s] == '0':  # si on est sur un carré non-assigné
                try:  #on choisit un nombre qui est dans les deux listes possibles, possibilities et values.
                    randomnumber = random.choice(''.join(set(possibilites) & set(values[s])))
                except IndexError:
                    randomnumber = random.choice(possibilites) # En cas d'erreur si la contrainte n'est pas satisfaisable
...
def listSwapPossibles(grid, values):
    swaps = []
    for bs in bigSquares:  # pour chaque carré 3x3
        casesNonFixes = []
        for s in bs:
        ...
        for i in range(0, len(casesNonFixes)):
            for j in range(0, i):
                if i != j and possible_valid_swap((casesNonFixes[i], casesNonFixes[j]), values):  ##considere uniquement un swap si les cases on des valeurs en commun
...
#But : prendre une grille et un couple de cases, et retourné une nouvelle grille ayant les meme éléments mais avec
...
# Successeur valide : retourne True si les  deux cases ont un échange compatible (a peut aller dans B et b peut aller dans A)
def valid_swap(grid, pair, values):
    """return valueA in square B && value B in square A"""
    return grid[pair[0]] in values[pair[1]] and grid[pair[1]] in values[pair[0]]

# verifie que deux cases ont des valeurs possibles en commun
def possible_valid_swap(pair, values):
    """return valueA in square B && value B in square A"""
    return len(set(values[pair[1]]) & set(values[pair[0]])) > 0
...
def solve(grid, nbReset = 0, nbIt = 0, bestConflits = 10000, nbSwaps = 0):
    originalGrid = grid_values(grid)  # on traduit le string en grille
    values = parse_grid(originalGrid)
    if not values:
        return False  # la grille ne correspond pas a un sudoku solvable
    swaps = listSwapPossibles(originalGrid,values)  # on genere les échanges possibles
    randomlyFilledGrid = fill(originalGrid,values)  # on rempli aléatoirement les cases vide
    result = annealing(randomlyFilledGrid, swaps,values)  # algorithme de hill climbing
    if(result[2] < bestConflits):
        bestConflits = result[2]
    if result[0] == False and nbReset < 10:
        return solve(grid, nbReset + 1, nbIt + result[1], bestConflits, nbSwaps + result[3])
    else:
        return (result[0], result[1] + nbIt, bestConflits, nbSwaps + result[3])
...
def annealing(grid, swaps,values):
    nbTours = 1
    nbSwaps = 0
    ...
    while (True):
        ...
        if conflitsAvant == 0:
            return (grid, nbTours, 0, nbSwaps) ## pas de conflit, donc déjà résolut
        for s in shuffled(swaps):
            if not valid_swap(grid,s,values): continue  #exclus de la boucles les swap non-compatibles
            conflitsApres = nombreConflits(swap(grid, s))
            nbSwaps += 1
            if conflitsApres < conflitsAvant: #si c'est mieux, alors on garde le swap
                conflitsAvant = conflitsApres
                nbTours += 1
                onATrouveUnSwap = True
                t = alpha * t
                break
            else: #si ce n'est pas mieux, on pourrait peut-etre quand meme jumper (selon la température et le delta)
                delta = conflitsApres - conflitsAvant
                proba = math.exp(-delta/t) #probabilité de jumper
                r = random.uniform(0, 1) #notre tir aléatoire
             
                if(r <= proba): #si on jump
                    conflitsAvant = conflitsApres
                    nbTours += 1
                    onATrouveUnSwap = True
                    t = alpha * t
                    break
                else: #si on ne jump pas, alors on defait le swap et on cherche un autre
                    swap(grid, s)

        # comme on n'a pas résolut le sudoku, on cherche le meilleur swap possible
        if not onATrouveUnSwap or nbTours >= 600:  # on n'a pas trouvé de swap réduisant les conflits
