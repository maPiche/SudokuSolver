...
f = open('tabousearch.txt', 'a')  # fichier log de la performance
...
def solve(grid):
    ...
    if not parse_grid(originalGrid):
...
def hillClimbing(grid, swaps):
    ...
    while (True):
        ...
        for s in shuffled(swaps):
            if s in tabouliste : continue       ##exclus les échanges de la liste taboue
