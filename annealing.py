...
# IDÉE : meme chose que hill-climbing, mais on a une possibilité de jump avec la température, et si l'algo bloque et ne
#        trouve pas de solution, il fait un reset et recommence du début
import math
...
f = open('annealing.txt', 'a')  # fichier log de la performance
    ...
    for bs in bigSquares:  # pour chaque carré 3x3
        ...
        for s in bs:
            if filled[s] == '.' or filled[s] == '0':  # si on est sur un carré non-assigné
                ...
                possibilites = possibilites.replace(randomnumber, '')  # puis on retire ce nombre des possibilitées
...
def solve(grid, nbReset = 0, nbIt = 0, bestConflits = 10000, nbSwaps = 0):
    ...
    result = annealing(randomlyFilledGrid, swaps)  # algorithme de hill climbing
...
#      selon le principe de l'algorithme de simulated annealing.
#      retourne la grille solvée, ou False si l'algorithme échoue, et retourne aussi le nombre de transition d'état fait
def annealing(grid, swaps):
    nbTours = 1
    nbSwaps = 0
